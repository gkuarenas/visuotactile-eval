...
- **Inter-slab stiffness variability**: Substantial variation in absolute peak force was observed across replicates of the same blend — most pronounced in B2 (75:25), where one slab reached ~600 mN at 6 mm depth while the other two reached ~130–150 mN (~4× spread). B1 and B4 each exhibited one outlier slab (n3) that was measurably softer than the other two. This variability is attributed to batch-to-batch inconsistency in hand-mixed elastomer preparation: small deviations in mixing ratio by mass, incomplete degassing (trapped microbubbles reduce apparent stiffness), and cure-condition variation between sessions. Despite this, the hysteresis index remained consistent across replicates (SD ≤ 3.2% across all blends), indicating that energy dissipation behaviour is a more reproducible material property than absolute stiffness under these preparation conditions. Screening applications that rely on HI rather than peak force are therefore less sensitive to this source of variability.
